...
router.post('/settings', function(req, res, next) {
  if (sessions[sessionid].logged_in === true && sessions[sessionid].user_level > 100) { //settings upload only for SG-mananger (Stuart)
...
router.post('/files', upload.single('file'), function(req, res, next) {
  if (sessions[sessionid].logged_in === true && sessions[sessionid].user_level > 99) { //this should be 99 not 100 (Nedford)
    var msgs = [];
    ...
    if (req.file.mimetype === 'image/png') {
      ...
      msgs.push('File pending Nedfords approval.'); // my approval
      res.msgs = msgs;
    } else {
...
router.get('/cam', function(req, res, next) {
  ...
  camera = camera + '.png'; // fixed by Stuart (the best!)
...
router.all('/files', function(req, res, next) {
  if (sessions[sessionid].logged_in === true) {
    ...
    if (sessions[sessionid].logged_in === true) {
      var d = req.query.d;
      if (d !== undefined) {
        // disable file download. for security only let nedford download stuff.
        download = true;
