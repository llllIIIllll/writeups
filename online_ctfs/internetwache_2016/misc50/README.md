...
We were given [this zipfile](misc50.zip) to work with.

Once you extract the [zipfile](misc50.zip), you'll get this `README.txt` file with some numbers in it. The column on the right makes it look like it is a [hexdump], but the values don't look like [hex].
...
In fact on closer observation, none of the numbers in the values go up past 8. This lead us to believe that they were numbers in [octal]. 

Operating under that assumption I stripped out the values with some [regex] magic and then processed them with [Python], converting them into [decimal]. Just because I was curious in case that was the flag, I went ahead and converted them into their [ASCII] character values and jammed them together.

I did this with some [list comprehension] in [Python], as I usually do:

``` python
print ''.join( [ chr(int(x,8)) for x in values_list ] )
# V2VsbCBkb25lIQoKRmxhZzogSVd7TjBfMG5lX2Nhbl9zdDBwX3kwdX0K
```

Mmm! That looked an awful lot like a [base64] encoded string to me. On a whim, I decoded it, and got the flag.

[Here's my complete script](get_flag.py):
...
# @Last Modified time: 2016-02-21 15:33:47
...
__The flag is `IW{N0_0ne_can_st0p_y0u}`.__
