...
We were given [this zipfile](misc60.zip) to work with.

Once you extract the [zipfile](misc60.zip), you'll get this [`README.txt`](README.txt) file, which contains seemingly a lot of garbage.

After some poking at it I determined that it was a bunch of [base64] encoded strings (I could see the trailing `=` equals signs used for padding). So, I wrote something that would go through an decode it all.

It was not all [ASCII] characters, though... some of the [hex] didn't actually give me a printable character, it was still in its `\x` escaped form. I tried to print it anyway, and all of a sudden I had a bunch of [QR codes].
...
With my knee-jerk reaction to seeing a [QR code], I just took my phone out and scanned it. It gave me a letter `I`, which I could tell was the first letter of the flag. So, since I had a bunch of different [QR codes], I figured I would scan them all and build the flag out.

Admittedly I did it by hand, just scanning each one individually. It wasn't too bad to do since it was a short flag, but honestly I don't know any software tools that could scan a "plain text" [QR code] like that without some other setup (that I would probably have been too lazy to actually write).

[Here's the script I wrote](ape.py) to process out all these [QR codes] one by one:
...
# I cut up the lines a little bit and placed them in a separate file.
...
After a while of scanning with my phone, I was able to piece together the flag.

__The flag is `IW{QR_C0DES_RUL3}`.__
