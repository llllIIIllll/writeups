__Internetwache CTF :: Web60__
==================================

_John Hammond_ | _Sunday, Feburary 21st, 2016_ 


> __Description:__ Regular expressions are pretty useful. Especially when you need to search and replace complex terms.

> __Service:__ https://replace-with-grace.ctf.internetwache.org/ 

----------

For this challenge, we were presented with some web page that could do a [regular expression] "search and replace" for us. We could supply the [regex] pattern, the string to replace it with, and the content to search through.

I struggled with this one for a while, but then eventually just tried to think it through. It's a webpage, and it's running [PHP][PHP] (you could estimate or predict this, but you could seriously validate that claim if you wanted to hunt around the code and find that `ajax.php` page it is using). And if it is replacing stuff with [regular expressions] in [PHP], well, we can only assume it is using the [`preg_replace`][preg_replace] function.

So, I tried to do some research. [What were vulnerabilities with that `preg_replace` function?](https://www.google.com/search?client=ubuntu&channel=fs&q=php+preg_replace+vulnerabilities&ie=utf-8&oe=utf-8)

That got me a lot of good stuff.

Apparently, if you can control the whole [regular expression] pattern and the string to search through and even to replace with, you can get some [remote code execution].

[Check it out](http://www.madirish.net/402), [there is some](https://bitquark.co.uk/blog/2013/07/23/the_unexpected_dangers_of_preg_replace) funky `e` option you can tack on to the end of your [regular expression] pattern and then, for whatever reason, [PHP] will execute whatever string you "replace with".

So, we could run the search with `/^(.*)/e` as my [regex] pattern, `system('ls')` to replace with, and `literally anything` as my content... I could get the output of the [`ls`][ls] command!

I tried this, but I was returned a `Blacklisted keyword!` response. Hmm. I guess they protected against the `system` function? Uhh, okay. I tried other things like `exec`, `popen` and `passthru` (all usual system calls for [PHP]), but none of them were able to go through.

__Well, thankfully, _backticks_ were able to go through.__

So that became my exploit:

* `/^(.*)/e` for [regex] pattern
* ``ls`` for string replacement 
* and `anything` for the content.

That got me directory listing, and then I was able to see where the flag lived.

* `/^(.*)/e` for [regex] pattern
* ``cat flag.php`` for string replacement 
* and `anything` for the content.
...
__The flag is `IW{R3Pl4c3_N0t_S4F3}`.__
