...
Pat had solved this during the actual competition, so all credit goes to him for these points, but I'll continue to crank out these writeups.

We were given [this zipfile](misc70.zip) to work with.

Once you extract the [zipfile](misc70.zip), you'll get this [`dump.pcapng.gz`](dump.pcapng.gz) file, which is a [packet capture] file you can open in [Wireshark]. 

If you look through some of the packets, you'll see there is a [GET request] for a file [`flag.zip`](flag.zip). Because we saw that, we figured we could try to export all the objects that [Wireshark] found with `File -> Export Objects -> HTTP`. With that, we were able to successfully save a copy of that [`flag.zip`](flag.zip) file.


Once we opened up that [zipfile], we could clearly see a `flag.txt` file in there that we obviously wanted to read &mdash; but we weren't able to extract it, it required a password.

Well, when we were looking through the [packet capture][packet capture] [file](dump.pcapng.gz), that same [TCP] conversation where they were [GET][GET request]-ing the  [`flag.zip`](flag.zip) file, they had a header called `Authorization` with a value `Basic ZmxhZzphenVsY3JlbWE=`.

That trailing `=` equals sign really set off an alarm that that was a [base64] encoded string. We were able to decode it, and get some useful information.

``` python
>>> import base64
>>> base64.b64decode('ZmxhZzphenVsY3JlbWE=')
'flag:azulcrema'
```

Aha! We tried that as the password for the [`flag.zip`](flag.zip) file we were able to export... and then we were able to extract the `flag.txt` file!

__The flag is `IW{HTTP_BASIC_AUTH_IS_EASY}`.__
