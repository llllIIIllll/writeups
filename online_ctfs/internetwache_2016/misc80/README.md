...
We were given [this zipfile](misc80.zip) to work with.

Once you extract the [zipfile](misc80.zip), you'll get this [`dump.pcapng.gz`](flag.pcapng) file, which is a [packet capture] file you can open in [Wireshark].  

Like it says in the challenge prompt, there are a bunch of [GET requests] to get the `flag.txt` file, but none of them return anything &mdash; it is a [404 error] every time. I tried navigating to those pages in my browser, but I still got a [404]. I tried variations of `flag`, like without an extension, or with a `.php` extension or other nonsense.

I tried going to the host website itself, without specifying the `flag.txt` download, but even that got me a __Forbidden__ error. I was honestly bahsing my head against the wall here for a while.

I could see that all the [URL]s had this really weird hash-looking [hostname], so for a bit I tried to throw them at online hash crackers or determine what it could be by examining the length and other stupid stuff like that.

Eventually, I tried to look at the "hashes" like just [hex], like each bit was really an [ASCII] character. __That finally got me somewhere.__

So I stripped out all the [hostname]s with some [regex] magic and then processed them all into text. From there, I got an interesting paragraph:
...
Hmm. I looked around, but initially didn't see a flag in there. Then I looked a bit more closely; __it was put together by the first character of each line!__

I finished up with a some code that would scrape out the flag for me. [Here is my final script](get_flag.py): 
...
# @Last Modified time: 2016-02-21 16:34:51
