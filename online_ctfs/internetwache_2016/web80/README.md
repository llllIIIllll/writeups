__Internetwache CTF :: Web80__
==================================

_John Hammond_ | _Sunday, Feburary 21st, 2016_ 


> __Description:__ I stumbled across this kinda oldskool blog. I bet it is unhackable, I mean, there's only static HTML.

> __Service:__ https://0ldsk00lblog.ctf.internetwache.org/ 

----------

The challenge prompt told us that all this was a static [HTML] page for a website. Well, it told the truth... _kinda_.

If you open up the web page, you can get a hint: 
...
Oooh... [`git`][git]? Mmkay. Is there a [github] repository for this thing? I tried to hunt stuff down by giving [Google] `site:github.com internetwache`.... but I didn't really find anything specific to this challenge, like a [repo] with the name "0ldsk00l" or something.

I then moved on to the idea that the _webpage itself_ might be a [repository]. Just because I was curious, I tried going to `https://0ldsk00lblog.ctf.internetwache.org/.git/`...

... and I got a __Forbidden__ error. So it _must exist_, we just _can't access it_! 

I tried to do some more digging, though. I tried to go to `https://0ldsk00lblog.ctf.internetwache.org/.git/HEAD`... __and I could read that just fine__.

So okay, we didn't have the permission to index the whole directory, but we _did_ have read access to the files. We just needed to get the files...

Well, while I was looking at some of those [repositories] that might be related to the challenge, I was able to find the [Internetwache github page](https://github.com/internetwache). That had a really interesting [repository] called... [__GitTools__](https://github.com/internetwache/GitTools). 

I looked at it a bit and it sounded like there was a "Dumper" tool that did everything we needed; it would scrape all the [`git`][git] files out of a webpage that had a [`git`][git] directory. So, I downloaded it, and then fed it the address of the challenge webpage.

```
john@john-Latitude-E7440:~/cyber_team/tools/GitTools/Dumper$ ./gitdumper.sh 
USAGE: http://target.tld/.git/ dest-dir
john@john-Latitude-E7440:~/cyber_team/tools/GitTools/Dumper$ ./gitdumper.sh "https://0ldsk00lblog.ctf.internetwache.org/.git/" old_skool
```

Sweet. I watched it download the whole [repository]. __That's really freaking cool.__

At that point, I was able to move into the new directory and interact with it like a regular [`git`][git] repository.

Here are the commands that I ran to get the flag:
...
That brought back the `index.html` that was deleted and not in the [repository]. When I [`cat`][cat]ed the file, it was the same as the one online, so I figured I would try to go back and time with some commits and find an older version.

I tried to go from the most recent commit downwards; and I got the flag real quick.

```
git log 
git checkout 26858023dc18a164af9b9f847cbfb23919489ab2 # the second most recent commit
```

Now, to checkout the changes to the `index.html` file...

```
cat index.html
```

And the flag was in that version of the file!
...
__The flag is `IW{G1T_1S_4W3SOME}`.__
...
[QRcodes]: https://en.wikipedia.org/wiki/QR_code
...
[repo]:https://guides.github.com/activities/hello-world/
[repository]: https://guides.github.com/activities/hello-world/