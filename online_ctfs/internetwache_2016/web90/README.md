...
> __Description:__ Creating and using coperate templates is sometimes really hard. Luckily, we have a webinterace for creating PDF files. Some people doubt it's secure, but I reviewed the whole code and did not find any flaws.

> __Service:__ https://texmaker.ctf.internetwache.org 

----------

Caleb was the one who got this during the competition, so all credit goes to him for the points, but I'll keep cranking out these writeups.

When you connect to the webpage, you'll see you can generate a [PDF] with some [LaTeX] syntax. That's pretty cool... __but how do we break it?__

Since [LaTeX] is, on its own, a language, we can probably make it execute some system commands. [We did a little bit of research to figure out how.](http://stackoverflow.com/questions/3252957/how-to-execute-shell-script-from-latex)

Turns out, you can run [shell] commands with:

```
\immediate\write18{ COMMAND }
```

When we tested this, we were able to see the output in the bottom section of the webpage. So, we poked around the file system. Eventually, we found the flag in the parent directory.
...
__The flag is `IW{L4T3x_IS_Tur1ng_c0mpl3te}`.__
