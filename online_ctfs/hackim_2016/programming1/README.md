__HackIM 2016 :: Programming Question 1__
==================================
...
This challenge asks us to scrape [Twitter] for a [Twitter] handle, based off the content of a certain Tweet. We figured we could probably write something to do this for us... so we did!

Finding some modules to do this in [Python] was pretty easy; I initially found [TwitterSearch], but eventually settled on [Twython] because I had better luck connecting with my application keys.

Here is the script I ended up writing for it...
...
twitter = Twython(  
					# These will have to be filled in with your application keys
					'-----------------',
					'---------------------------------------------------',
					'---------------------------------------------------',
					'----------------------------------',
...
You have to give [Twython] some application keys for your own [Twitter] bot. Once you do that, you can simply search based off a query (amp up the number of responses, I just set mine to something crazy to like 900). We displayed them all and then looked for which tweets that matched the criteria of the challenge description.

__The flag is `@anyahotels`.__
