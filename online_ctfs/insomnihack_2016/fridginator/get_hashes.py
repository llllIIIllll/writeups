...
food = 'http://fridge.insomnihack.ch/food/'
users = 'http://fridge.insomnihack.ch/users/'
...
def get_from( the_url, the_string ):

	response = session.post(the_url, cookies = cookie, data = {"term": the_string, "csrfmiddlewaretoken":cookie["csrftoken"]} )
	hashed = response.url.split('/')[-2]

	first = hashed[:32]
	second = hashed[32:]

	print Style.BRIGHT + Fore.CYAN + the_url.split("/")[-2] + "\t" + Fore.YELLOW + the_string + "\t" + Fore.GREEN + first + " " + second


for i in range(  3 ):
	possibilities = itertools.combinations_with_replacement(characters,i) #1
	for possibility in possibilities:                                                      #2 
		the_string = ''.join(possibility)
			
		get_from( food, the_string )
		get_from( users, the_string )
		

		# response = session.post(url, cookies = cookie, data = {"term": the_string, "csrfmiddlewaretoken":cookie["csrftoken"]} )
		# hashed = response.url.split('/')[-2]


		# print Fore.YELLOW + the_string + " " + Fore.GREEN + hashed
