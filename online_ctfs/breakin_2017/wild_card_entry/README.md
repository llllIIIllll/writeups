...
I _really_ liked this challenge; it was some fun [SQL injection] and had an interesting trick with [PHP].
...
I did this with some classic [`UNION SELECT`][UNION SELECT] testing.
...
A while back I put together my own personal ["SQL injection cheatsheet"](https://github.com/USCGA/miscellaneous/blob/master/SQL_INJECTION_CHEATSHEET.md), and I used that heavily to leak information out of this database.
...
___Note that I did not use quotes here! That is what gave me the success for this query. I was able to use quotes for all the previous queries.__ You can use that same kind of style for all the other queries (wrap other `SELECT` statements inside of parentheses).
...
![access_denied.png](access_denied.png)
...
After some banging my head against the wall (for quite a long time) and searching for [phpMyAdmin] bypass vulnerabilities (for quite a long time) I realized that, yeah, the [PHP] was seemingly filtering out the `root` username. Even if the wrong password were supplied for that account, I would get the same __Access Denied!__ error... like it was purposely finding the username and acting against it.
...
The `%00` is a [URL-encoded][URL-encoded] [null byte]. It means nothing; but [PHP] will buy it.

I had to enter this with a proxy (___you can't enter it just as text in the form entry field!___) like [TamperData].

![tamper.png](tamper.png)
...
__The flag was: `07f663cbd03849d23e177a9348058722`__
