...
This challenge was peculiar and I ended up just doing a small tweak to get the flag to spit out.

Once you download the binary, if you run it, you'll see it asks for a special password and it won't move on unless you get it right. 

I saw in [IDA], though, that the program did a lot more than just that check. It looked like it would try and communicate with a server, and I didn't honestly care to analyze it more since I wanted to try and get past that password check.

I did this in [`gdb`][gdb] by just breaking at the string compare, setting the `eax` register to `0` (as if the string comparison evaluated successfully, essentially bypassing the check), and then watching the flag get spit out.

It is done pretty easily with this [`gdb`][gdb] script:
...
Then we can enter anything for our string, so our [`get_flag.sh`](get_flag.sh) script looks real simple, like this:

``` bash
#!/bin/bash

echo "" | gdb -x a.gdb a.out
```

![flag.png](flag.png)

__The flag was: `c47aced5c6004d6c868fffd74dc6f61b`__
