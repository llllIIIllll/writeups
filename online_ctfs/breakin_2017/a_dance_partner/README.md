...
Personally I thought this challenge was really clever, and required some out of the box thinking (and some suspended disbelief).

Once you download the files, you should have a [PDF] file and a [JPG] file with essentially the same content. It looks odd; a lot of grayscale color and boxes, but with a green box just in the very center. You could maybe mistake it for a strange [QR code]... but no, it's not that.

![Question.jpg](Question.jpg)

The hint is in the challenge prompt. They say with this line: "she teaches him only easy moves, such as moving forward, backward, right and left" ... the flag is built by moving around the image, and getting character values out of the [RGB color] values. Since they are all grey, the values are all the same... and they may land nicely in the [ASCII] range. 

The next question is what directions do we move in and when. That's where some of our reconnaissance comes in: the image has some `Comment` metadata with the directions. You can see this with the [`exiftool`][exiftool] tool.

![directions.png](directions.png)

The interesting thing here is that these are [ANSI escape code sequences](https://en.wikipedia.org/wiki/ANSI_escape_code). The `A` refers to hitting the arrow key up, `B` refers to down, `C` right, and `D` left.

Now it was just a matter of scripting this and cranking out the values. I took a look at the image and found each the box dimensions so I knew where to "start" in the image and how much to move by.
...
# comment = ... # I AM EXCLUDING THIS IN THE CODE PREVIEW FOR BREVITY.
# THE FULL STRING IS INCLUDED IN THE FILE AVAILABLE IN THIS REPOSITORY
...
That spit up a bunch of characters, seemingly garbage.
...
But, the challenge prompt said: ___If the length of answer string exceeds 255, you need to submit the md5sum of the flag___.

That's it! We can just `md5sum` this output. __MAKE SURE__ you do not have a newline here; that will not be accepted!

I combine my [Python] script with some [`bash`][bash] nonsense to put together my [`get_flag.sh`](get_flag.sh) script:
...
__The flag was: `f524415e198cbc8983ac0bed3d0cbcef`__
