...
> John Hammond | Monday, January 30th, 2017
...
This challenge at its surface looks more like a [reverse engineering] challenge, but it was supposedly lumped into the "miscellaneous" category. Now that I have solved the challenge, I would call it a [steganography] chalalenge.

When you download the source code, you'll notice it has all this weird spacing.
...
Looking at the whitespace within [Sublime Text], it looks like there is a clear distinction between tabs and spaces.

![whitespace.png](whitespace.png)

I had seen a challenge before in a previous [CTF][CTF] (I think it was [School CTF](https://github.com/USCGA/writeups/tree/master/online_ctfs/schoolctf/empty_spaces_COMPLETE)) where the distinction in whitespace was actually meant to be binary. Maybe in this case, the tabs were meant to be 1s and the spaces 0s, or vice versa.

To test the theory I started to tinker.

First, I removed all the characters that weren't new lines by changing them to another character (an underscore, in this case).

```
tr -c '\t \n' '_' < program.cpp 
```

Then, I decided to crank treat all of the tab characters as 1s.

```
tr -c '\t \n' '_' < program.cpp | tr '\t' 1
```

That means all the spaces have to be 0s.

```
tr -c '\t \n' '_' < program.cpp | tr '\t' 1 | tr " " "0"
```

And then I'll remove all the remaining underscores.

```
tr -c '\t \n' '_' < program.cpp | tr '\t' 1 | tr " " "0" | tr -d "_"
```

I ended up with some results that looked like this:
...
I thought this looked really promising, since most of the lines were the same length in this binary rendition. The lines that had just a `1` on them must be the blank lines that just have indentation to look like source code, and there were some anomalies at the end because of the trailing comment.

I cut those out to have some more uniform values.

```
tr -c '\t \n' '_' < program.cpp | tr '\t' 1 | tr " " "0" | tr -d "_" | strings -n 3
```

Now, with [Python], I whipped up some list comprehension to string these values together as characters.
...
Aha! That has to be it. Clever [stego]!

__The flag was: `WpUAItsadmhak`__
