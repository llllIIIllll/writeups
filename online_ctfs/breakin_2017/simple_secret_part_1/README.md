...
So this was not a reverse engineering challenge... like at all. 

This was a simple [`strings`](strings) challenge. 

You could download the executable and simply run [`strings`][strings] on the binary to get the flag.

```
strings a.out
the_flag_is_bond_007
```

The [`get_flag.sh`](get_flag.sh) script is an easy [`strings`](strings) and [`grep`](grep) combo.
