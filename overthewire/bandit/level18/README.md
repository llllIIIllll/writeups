...
... but this [shell] kicks us out! Once we enter the password, it says __Byebye!__ and it logs us out!

This is part of the challenge; the challenge prompt tells us that someone has modified the [`.bashrc`][.bashrc] file to log us out when we log in with [`ssh`][ssh].

So if you have not heard of it before, I suggest you do a little bit of reading on what the [`.bashrc`][bashrc] file is... but, for an extremely brief description, it is really just like a "profile" for your [`bash`][bash] [shell], in that it is a script that is run every single time you start [`bash`][bash]. So, if it logs us out when we try and get a [shell], we don't have access to a [shell] to be able to change the file and fix it, right? So we're kind of screwed, right?

Well -- I say it every time, but you should seriously look through the [man pages] for all the commands listed in the "Commands you may need to solve this level" section. 

If you are diligent and thorough when looking at the [`ssh`][ssh] [man page]. you should notice that you can have [`ssh`][ssh] _run a command_ once it is connected to the remote machine. It just takes the command as another argument. 

We can take advantage of this to try and run a command before we get kicked out of our connection. Try it:

```
$ ssh bandit18@bandit.labs.overthewire.org ls
```

Notice my [`ls`][ls] command tacked on at the very end? Once you enter the password, you should be able to see the output:

```
readme
```

Aha! The challenge prompt told us there is a `readme` file in the [home directory] that has the password for the next level. This must be that file! Let's try and [`cat`][cat] it out, the same way we ran the last command (as an argument to [`ssh`][ssh]).
...
__The password for bandit19 is `IueksS7Ubh8G3DCwVzrTd8rAVOwq3M5x`.__
