...
The challenge prompt tells us that there is a [setuid] binary in the [home directory]. If we [`ls`][ls], we should be able to see it.

```
bandit20-do
```

It may be displayed with a red background in your shell. This is how [`ls`][ls] tries to colorcode a [setuid] binary.

Try taking a look at it with `ls -l`.

```
-rwsr-x--- 1 bandit20 bandit19 7370 Nov 14  2014 bandit20-do
```

You should be able to see some interetsing things.

For one thing, the leftmost name, the _owner of the file_, is `bandit20`. But we are only logged in as `bandit19`! You can the rightmost name is `bandit19`, which means the file belongs to the _group_ of "bandit19", the same group that our user is a part of.

Take a look at the [file permissions] on the very left side. You can see the owner has read, write, and execute permissions, but the execute bit is portrayed with a weird `s` symbol. __That denotes the fact that it is a [setuid] executable!__.

You can also see that anyone in the "group" has execute permission; that is why we as the `bandit19` user can run the program.

So what the heck is a [setuid] binary? You should do some research on it, but it is a program that runs _with the privileges of the program owner_. So, even though we are the `bandit19` user, whenever we invoke the `bandit20-do` program, all the code that runs and gets executed is by the _owner_ of the program.. in this case, the `bandit20` user!

You can see this in action if you run the program. The challenge prompt tells us that if we run the program without any arguments it will explain how to use it:
...
So the `bandit20-do` program will run whatever command we specify (as an argument, it looks like) as another user (in this case, the `bandit20` user).

Let's try that example they give us:
...
Look closely at this output! Our `uid`, our _[user id]_ is for `bandit19`,  but our `euid`, or our [effective user id] is that of the `bandit20` user. Because of the way the [setuid] binary works, we are _effectively_ the `bandit20` user whenever we run the program!

So let's try and run a different command, just to further drive the point home:
...
Aha! See? Within the confines of the program, we are essentially the `bandit20` user, so we have all the rights and privileges of that user.

With that in mind... since the `bandit20` user is the user for the next level, what is to stop us from reading the password for the next level? We know all of the passwords are stored in the `/etc/bandit_pass` directory... and since we have the privileges of the `bandit20` user, we should be able to read his password!
...
There we go! We were able to take advantage of the fact that we had the privileges of the `bandit20` user, and find his password!

__The password for bandit20 is `GbKksEFF4yrVs6il55v6gwY5aVje5f0j`.__
