...
The challenge prompt tells us that there is another new [setuid] binary in the [home directory]. If we [`ls`][ls], we should be able to see it.

```
suconnect
```

Just like before, this is displayed with a red background, to denote that it is a [setuid] binary.
 
The challenge prompt tells us that this program will "make a connection to localhost on the [port] you specify as a [command-line] argument." So, if we _don't_ pass it an [argument], it tells us:
...
Ah, cool, it explains that you have to give it a [port number] as an [argument], and once you do, it will try and connect to it. If the program's connection receives the password for the current level, it will give us the next levels password.

The __notes__ in this challenge prompt explain that we will need to connect to this level with [`ssh`][ssh] _twice_ -- once to start the listening server, and then again to run the new `suconnect` [setuid] binary.

Let's go ahead and connect again in a different terminal window. Same log in as before...

```
ssh bandit20@bandit.labs.overthewire.org
```

Now, the `suconnect` [setuid] program says it will connect to localhost (this current machine) on a [port] that we specify. It has to have something to actually connect to, right? There has to be something ___listening___ on the other side that it can connect to; there must be a _server_.

Historically, you have been using [`netcat`][netcat] to _connect_ to a server just as a __client__... but, [`netcat`][netcat] can also be used to _listen_ as a __server__!

If you check out the [man page] for the [`netcat`][netcat] command you should be able to find out the [argument] to do that very easily, just by reading a bit.
...
-l      Used to specify that nc should listen for an incoming connection rather than initiate a connection to a
             remote host.  It is an error to use this option in conjunction with the -p, -s, or -z options.  Addi-
...
The [argument] is `-l`, and it will of course need a [port number] specified as another [argument] so it will know what [port] to really listen on.

We'll have to decide on a [port number]. Since the first `1000` are typically reserved for other services and protocols, I just chose the number `4444`. 

So in one of our connected terminals, we can run...
...
You should see it "hang".... nothing displayed, nothing happening, just a blank line. It spun up our server, and it is now just waiting for an incoming connection. 

In the other terminal window, let's try and connect to that [port number] with the `setuid` binary.
...
Aha! No disconnect, it didn't just close out right away... it must have made the connection!

Now, the challenge prompt says that _if the __`suconnect`__ program receives the password_, it will grant us the next one. So the `suconnect` has to ___RECEIVE___ it... so it must be _sent_ from the "server" side! The terminal that we set up as the server, the one that we ran the `nc -l 4444` command in -- that is where we should paste the current password. Let's try it!

```
GbKksEFF4yrVs6il55v6gwY5aVje5f0j
```

On the `suconnect` terminal, I see the output:

```
Read: GbKksEFF4yrVs6il55v6gwY5aVje5f0j
Password matches, sending next password
bandit20@melinda:~$ timed out waiting for input: auto-logout
```

And back on the server terminal:
...
__The password for bandit21 is `gE269g2h3mw3pwgrj0Ha9Uoqen1c9DGr`.__
