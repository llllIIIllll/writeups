...
The `data.txt` file is again in the [home directory]. When we [`cat`][cat] that out, it looks like a bunch of garbage.

Like the challenge prompt says, the file contents have had their letters rotated by 13 positions in the alphabet. This is a very very trivial form of [encryption], called the [Caesar Cipher][Caesar Cipher] (arguably, it is not even "encrypted" and this would nto be called "encryption"... it's just a cipher). Having a computer "decrypt" it is an easy task; we could do it on our own, but [automation is divine]. 

There are [a ton](http://www.xarg.org/tools/caesar-cipher/) [of online](http://www.simonsingh.net/The_Black_Chamber/caesar.html) [tools to](http://practicalcryptography.com/ciphers/caesar-cipher/) [do this](http://www.braingle.com/brainteasers/codes/caesar.php) [for us](http://www.dcode.fr/caesar-cipher), but it's nice to have a tool that can do it on its own and we don't have to trust those external resources to actually do it correctly.
 
So there is one available in the [Ubuntu][Ubuntu] [repos]. If you type in `rot13` in your [command-line] prompt, your should get a notification that that is available in the [bsd-games](https://launchpad.net/ubuntu/+source/bsdgames) package. Let's install that!

__You obviously can't run this on the [Bandit] machine... so install it on your own machine! :)__

```
$ sudo apt-get install bsd-games
```

Then once it has finished installing you should be able to run `rot13` (it has the synonym `caesar`) and you can enter any text into [standard input] to be rotated by 13 characters on [standard output]. If you check out the [`man`][man] page for `rot13`, notice that the other [argument] you give it can change the number of characters you rotate by (though 13 is the default).

Since it takes standard input, we'll just just have to [pipe] the string we need to "decrypt" into it. Copy the string from a [terminal] you have [`ssh`][ssh] connected to the [Bandit] level into a [terminal] just on your local machine, and [`echo`][echo] it in.
...
Awesome; it is an easy task, just a matter of recognizing the issue and knowing the tool for the job.

__The password for bandit12 is `5Te8Y4drgCRfCx8ugdwuEX8KFC6k2EUu`.__
