...
The `data.txt` file again is in the [home directory]. When we [`cat`][cat] that out, it looks like a bunch of garbage.

Like the challenge prompt says, the file contents is apparently [base64] encoded. This is a pretty common thing in [CTF]s, so it is good to be acquainted with [base64] and know how to both encode things and decode things. 

If you've never seen any [base64] stuff before, I recommend you read up on it; the jist of it that you need to know, though, is that it will take text and encode it in anything with a mix of uppercase and lowercase letters, as well as a `/` forward slash and `+` plus sign. __The encoded data will always have a length that is a multiple of four. _If the encoded data does not have a length that is a multiple of four, a number of `=` equals signs will be appended to the end to ensure the encoded data DOES have a length of four_.__

So, if you ever see anything random text with that description (a mass of letters and numbers with maybe some equals signs at the end), then it is likely [base64]. 

You can convert in and out of it with simple tools. The that [Bandit] seems to recommend (based off of there "Commands you may need to solve this level section") is simply the [`base64`][base64] command at the command-line. 

The [`base64`][base64] command will take a file as an [argument], and then encode or decode (encode by default) its contents based off the other given options.

You can read more about it in the [`man`][man] pages (you should do that right now), but here's a simple breakdown.
...
``` bash
$ base64 a_file.txt
# YOU WOULD SEE THE BASE64 ENCODED DATA OF THE a_file.txt CONTENTS ON STANDARD OUTPUT
```
...
``` bash
$ base64 -d a_file.txt
# YOU WOULD SEE THE BASE64 DECODED DATA OF THE a_file.txt CONTENTS ON STANDARD OUTPUT
```

__Note that you just need the `-d` argument to decode stuff.__

That means our challenge should be as simple as...
...
Now because I feel like you should know more than just one way to do things, please know that you can do this in [Python]. And since you will _hopefully_ be writing [scripts] one day, you should know _how_ to do this in [Python].

In [Python], you can use the [`base64`][Python:base64] module. The functions to encode and decode are, correspondingly, `base64.b64encode(<string_variable>)` and `base64.b64decode(<string_variable>)`

Those functions will return a string with the encoded/decoded data. Easy enough, right?


__The password for bandit11 is `IFukwKGsFW8MOq3IRFqrxE1hxTNEbUPR`.__
