...
The challenge prompt tells us we have to submit _our current password_ to the [port] 30000 on our current machine, localhost.

Well, we know we can connect to that [port] with [`netcat`][netcat], right? If you've never heard of [`netcat`][netcat] before or are just now using it for the first time, I obviously recommend do some reading on it. It essentially just lets you connect to some host on some port (and you can do a lot more with it, but that's really the jist of it). Just read the [`man`][man] page.

If you do check out the [`man`][man] page, you'll know the syntax of the command goes with two [arguments] for the host and [port number].

So let's connect to the port!
...
Now when we do this, it looks like it just hangs. But you've actually made a connection. It's just waiting for your input. 

Let's feed it the password for the current level.
...
__The password for bandit15 is `BfMYroe26WYalil77FoDi9qh59eK5xNr`.__
