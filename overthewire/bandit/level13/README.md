...
> The password for the next level is stored in __/etc/bandit_pass/bandit14 and can only be read by user bandit14.__ For this level, you don’t get the next password, but you get a private SSH key that can be used to log into the next level. __Note: localhost__ is a hostname that refers to the machine you are working on
...
Let's check out the current directory with [`ls`][ls] and see what we've got to work with.
...
Oooh! It looks like it gives us a [`ssh`][ssh] [private key]. If you know a bit more about [`ssh`][ssh] (hint: read the [`man`][man] pages), you'll know you can use this as kind of an identification file to log in with. It removes the need for a password when you [`ssh`][ssh].

You use this with the `-i` [argument], and specify the file you are using as your [private key], then continue your regular [`ssh`][ssh] syntax.

Note that this challenge prompt does tell you that "localhost" will refer to the current machine you are on; so you can essentially "level-up" on this [Bandit] box.

__Remember to specify the `bandit14` user as the user you want to log in as!__
...
Let's save it and move on to the next level!

__The password for bandit12 is `4wcYUJFw0k0XLShlDzztnTBHiqxU3b3e`.__
