...
Log in as usual to the account with the [private key] you retrieved from the previous level.

```
ssh -i bandit17.key bandit17@bandit.labs.overthewire.org
```

This challenge prompt tells us there are two different files in the [home directory], `passwords.old` and `passwords.new`. The password for the next level is the only line that is different between the two of them.

Let's take a look at the files by [`cat`][cat]-ing them out.

```
$ cat passwords.old 
zpR0btSjCaa1i8EFGmVcgDjg9XZ7c1rS
vU3RHnU4UymFHVPa0k2zs7I4MiYhV4bH
NDBMRzuhbwjzXGlTYzljpCtq1odKCtF5
9ECOmwIszk6Wg78EpbKVXXt7228FaALS
VMccoT1cQSieloUoMYY1dvOgZaj3VbA2
YRZoMPJWXM9Ln3BIHXgSMngu1R13sPgo
sGun8jcCZEpwlU4euZXyxRMdJPGHoyfM
KoRKHM5xUKVdMEyua4U62warUxqWzmrU
bSi38QejNLImaklMyJHhryMDOxemf04i
4iQ0ZjFYSTWBvBcxN8Bc5JqlIKpUNDh3
vK0SdhdX5dpeEUYB3coIADkMXJFrbIsU
n6BKgAcxqhOHMg1XxmVEzud2fUgn9OjR
9Oq0haOWJVCAG6PAJByFTixoCY4X8FD6
...
```

Ah, there are a lot of lines. How many, in fact?
...
`100` lines. Okay; let's not parse through that by hand.

Again, you should have looked through the "Commands you may need to solve this level" section. I believe that at this point you have already used the commands like [`cat`][cat], [`grep`][grep] and [`ls`][ls]... the only thing that remains is [`diff`][diff]. 

If you check out the [man page] for the [`diff`][diff] command you will see that it "compares files line by line" and should show the differences between two files. Well, this is perfect!

It takes the files as arguments, so let's try and run it with the two `password.old` and `password.new` files. 
...
That notation, the `<` notes that this is the line in the FIRST specified file (in our case, the `passwords.old` file) that is changed to the line given by the `>` symbol (and this is the line in the SECOND specified file, our `passwords.new`).

So, that different line must be `kfBf3eYk5BPBRzwjqutbbfE887SVc5Yd`. That must be the password for the next level! 

Note that when you try and log in, you are greeted with __Byebye!__ and kicked out of your `bandit18` [shell]. _This is okay!_ This is exactly what is explained in the challenge prompt; it explains to us that that _is_ part of the next challenge. 

__The password for bandit18 is `kfBf3eYk5BPBRzwjqutbbfE887SVc5Yd`.__
