...
_John Hammond_ | _Thursday, January 7th, 2016_ 
...
Our prompt tells us we should be working with this file called `data.txt`. 

If we [`ls`][ls], we can clearly see that file in our [home directory]. Let's [`cat`][cat] it out:

``` bash
$ cat data.txt
```

Oh, boy, holy crap -- that is a lot of data! How can we easily process that?
...
We can do that with the awesome [Linux] utility called [`grep`][grep], for "globally search a [regular expression] and print". 

It is essentially a [command-line] search tool. We can interact with it by supplying a file and what we want to search for. It will either take these options as [arguments], or it can read from [standard input] and we can [pipe] the data into it.

That's two options:

__1) with [arguments]__
...
Either way you do it, you still get your flag. And we can move on to the next level!

Now, [`grep`][grep] is one awesome tool. Always have it in your back pocket and know that you can very easily search through large amount of data through the command-line. __I recommend you check out its [`man`][man] pages and see what more good stuff you can do with it.__

__The password for bandit8 is `cvX2JJa4CFALtqS87jk27qwqGhBM9plV`.__
