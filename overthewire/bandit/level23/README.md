...
More [`cron`][cron] stuff. Let's head over to that `/etc/cron.d` directory as explained in the challenge prompt and check out the files.
...
Looks like this time we want to be reading the `cronjob_bandit24` file. Let's [`cat`][cat] it out.
...
Just like all the recent challenges, it invokes a [script]. Looks like the one it calls this time is `/usr/bin/cronjob_bandit24.sh`. I'll check it out with [`nano`][nano] again.
...
We should be able to make sense of this [script] by now, right?

The first line is our usual [sha-bang] line. We set another variable, by [command substitution], getting our current username. Since this [script] is invoked by [`cron`][cron], the user calling it is really `bandit24`. We can expect that to be the value of that `myname` variable.

Then it changes directory to a new location; one that we can now deduce to be `/var/spool/bandit24`. It displays onto [standard output] that it is going to "execute and delete all scripts" in that directory.

This is the first time (if you have been going through this [Bandit] writeups because you are new to this "cyber" thing and [Linux] and [`bash`][bash]) that we see a [`for` loop][for loop]. This is very common technique of looping over lots of data within a program or a [script]. In this case, it loops over every file in the directory and executes each one. [`for` loops][for loop] can be used for many different things, like looping through files (as seen) or indices in an array, or even counting. 

Regardless, now we know that the `cronjob_bandit24.sh` executes any [script] that is present in that `/var/spool/bandit24` directory.

___Hmmm.___

That gives us a little bit of control, right? What is to stop us from _making our own [script]_ within that directory, to be ran and executed as the `bandit24` user? That would give us the ability to see the next level's password, right?

Let's hop on over to that directory.

```
$ cd /var/spool/bandit24
```

Now, let's create our very own [`bash`][bash] script.

```
$ nano our_script.sh
```

What do we need to put in our script?

Well, it obviously needs a [sha-bang line], like every script in the world. Then, just like we have seen in other levels, we can [redirect] the next levels password into a file that we can read... ___like something in the [temporary directory]___! We will have decide what name it will have, though. Since we are writing _our_ own [script], we can decide anything we want!
...
This out to put the next password in the `/tmp/our_winner.txt` file!

We can break out of [`nano`][nano] with `Ctrl+X`. 

Now, we have to mark our script executable, so it will actually be run by the [`cron`][cron] job (rather than just deleted without our results.) We can do that with the [`chmod`][chmod] command.

```
$ chmod +x our_script.sh
```

Give it a bit, and then it should be executed by the script [`cron`][cron] in the background. 

Finally, we should be able to view our password!

```
$ cat /tmp/our_winner.txt
```
...
__The password for bandit24 is `UoMYTrfrBFHyQXmg6gzctqAwOmw1IohZ`.__
