...
Ah, this is another [`cron`][cron] challenge, just like the last one. The challenge prompt states that the files are in the `/etc/cron.d/` directory, so let's change directory over there and view the files.
...
Since last level's file was `cronjob_bandit22`, we want to look at the one for this one (that leads us to the password to the _next_ level): `cronjob_bandit23`
...
Okay, this looks like it invokes another [`bash`][bash] [script], just like the [`cron`][cron] job did in the last level. Let's check out that script. I'll open it in [`nano`][nano] this time, the simple [command-line][command-line] [text editor].

```
$ nano /usr/bin/cronjob_bandit23.sh
```
...
We see the [sha-bang line] at the very start of the file, telling [Linux] that we do want [`bash`][bash] to interpret this script.

The next couple of lines do some [command substitution]. That `$()` syntax, with some commands wrapped inside the parentheses, is one way of denoting [command substitution] within [`bash`][bash]. Essentially, that syntax becomes _replaced_ with the output of those specified commands. So, what is happening with those commands? We can test them out and see what they do, if we are unsure...
...
The [`whoami`][whoami] command just returns the currently logged in user. That is what is being stored in the `myname` variable, according to this `cronjob_bandit23.sh` script. Keep in mind, though, that the [`cron`][cron] job is being _executed_ by the `bandit23` user; we could see that it in the specific [`cron`][cron] job file. So, in that case, the `whoami` command _really_ returns `bandit23`... __the user for the next level!__

What does the next [command substitution] do, what is the variable `mytarget` being set to? We can explore those commands if we don't know them.

Since all of this is a [script] that we can read and understand, there is nothing stopping us from running the commands on our own and seeing what they do.

```
$ echo "I am user $myname"
```

Ah, this [`echo`][echo] command would just display onto [standard output] "I am user bandit23". We can change up our command (used for testing) to reflect that.
...
And it looks like this is [piped] to another command, the [`md5sum`][md5sum] command. So that [piping] means the output of the command is sent as the [standard input] to the following command ([`md5sum`][md5sum] in our case). 

The [`md5sum`][md5sum] command essentially computes an [MD5][MD5] [checksum]. [MD5] is an algorithm and widely used hash function that produces a 128-bit [hash] value. [MD5] was initially designed to be used as a cryptographic [hash]function, but now it has been found to suffer from extensive vulnerabilities. Regardless, in our case, it is just being used to generate a [hash], or a random set of characters.
...
See how it outputs those crazy letters and numbers? That's our [MD5][MD5] [checksum]. 

Notice, though, there is a hyphen `-` just at the very end of the output. It looks like the script tries to remove that, by again [piping] into the [`cut`][cut] command, which lets us specify a delimiter (in this case a space character ` `) to split each line by. Then it takes the first field in the split line... yielding just the [MD5][MD5] [checksum] on its own.
...
So that means that that [checksum], the `8ca319486bfbbc3663ea0fbe81326349` string must be the value of the `mytarget` variable.

What does the rest of the `cronjob_bandit23.sh` [script] do?
...
Ah, it displays out the variables... but we would not be able to see that output, because the [script] is run in the background by the [`cron`][cron] service!

But then it does [redirect] some [standard output] to a file.

Oh! It redirects the `/etc/bandit_pass/$myname` file to the `/tmp/$mytarget` file... and `$myname` is the value `bandit23`, and `$mytarget` is the value that we got from that [MD5][MD5] [checksum]! 

If we replace those values, that means that... ___the level for the `bandit23` user, that password is stored in the `/tmp/8ca319486bfbbc3663ea0fbe81326349` file!___

Let's [`cat`][cat] out that file:
...
__The password for bandit23 is `jc1udXuA1tiHqjIsL8yaapX5XIAI6i0n`.__
