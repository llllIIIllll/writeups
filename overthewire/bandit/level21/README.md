...
This challenge prompt tells us that there is a program that is being run at a scheduled interval, set by [`cron`][cron]. If you haven't heard of [`cron`][cron] before, I suggest you do at least a little bit of reading and researching, but it is exactly what it is described to be: a time-based job scheduler. You can use it to schedule when programs should be run on your computer, scheduled for any time in the future, or to be ran every day, or every week, or any other configuration you would like.

The challenge prompt tells us that the configuration for [`cron`][cron] is in the directory `/etc/cron.d/`. Let's head over there and explore.

```
$ cd /etc/cron.d/
```

And lets see what it is in the directory...
...
Oh, woah, there is a lot of stuff. Considering we are on the [Bandit] level to try and get the `bandit22` user account, lets check out the file `cronjob_bandit22`
...
Hmm. This looks like a special kind of syntax, specific for a [`cron`][cron] scheduled job. [You can do some research](http://askubuntu.com/questions/2368/how-do-i-set-up-a-cron-job) to determine what this syntax is really all about, but the first couple of stars/asterisks `*` really just denote whether or not they are repeated each `minute`, `hour`, `day-of-month`, `month`, and the `day-of-week`. The last two fields should be a user to run a command as, and finally, the command that is being run.

So, at the very far right, we can see the command being run. In our case, it is 

```
/usr/bin/cronjob_bandit22.sh &> /dev/null
```

So if we try and dissect this, we know that the real command here is `/usr/bin/cronjob_bandit22.sh`.  This is a [`bash`][bash] script! You can tell because it has a `.sh` [file extension], which is the typical extension for a [`bash`][bash] script.  

If you haven't heard of a [bash script], it is essentially a plain text file that groups a ton of [`bash`][bash] command into one. It lets you execute commands in bulk; one after another, they will be executed in sequence. 

The real file name is `cronjob_bandit22.sh` while the path is `/usr/bin/`... that file is in _that_ directory!

So we invoke that script by just entering its name, `/usr/bin/cronjob_bandit22.sh`, but what are all the weird things that follow?

The `&` symbol notes [that the command is run in the background](http://www.cyberciti.biz/faq/linux-command-line-run-in-background/). The `>` symbol is part of [redirection]. It says that all the output of the command will be _redirected_ into a different file descriptor or file; that `/dev/null` file is a special file, like a digital trash can. `/dev/null` is like a black hole; all data that is sent there goes away. Redirecting standard output to `/dev/null` is a common tactic to try and hide the output of a command.

That is everything that is happening with that command. But since it is calling a [script], we want to know what the _script_ does, too! Let's try and check out what that file does, we can just [`cat`][cat] it out since a script is just a plain text file.
...
You can see output here that is the contents of the `/usr/bin/cronjob_bandit22.sh` script. 

Let's go through it line by line, just to explain everything it is doing.

```
#!/bin/bash
```

The first line is a special line that is typically part of every single [script] you will ever see. It is what is called a [sha-bang line]. It is what tells the [operating system] which program or executable you want to be the 'interpreter' of your [script]. Since this is a [`bash`][bash] script, we obviously want the [`bash`][bash] binary and executable to be the program that reads and runs the commands! So, we need the path of the [`bash`][bash] program, which we can find with the `which bash` command. Then you tack on a `#!` to have special notation and denote that this really is the [sha-bang line].
...
This next line uses the [`chmod`][chmod] command that you saw and used in a previous level. It is used to change the [file permissions] on a file, and again they use the [octal file permissions] representation. The `644` means that it is `6`: readable and writeable by the owner; `4`: readable by those users in the same group; and `4`: readable by _everyone_.

It makes these changes to a file, given the filename `/tmp/t7O6lds9S0RqQh9aMcz6ShpAoZKF7fgv`. That makes this file readable by everyone... so we should be able to view the contents!

```
cat /etc/bandit_pass/bandit22 > /tmp/t7O6lds9S0RqQh9aMcz6ShpAoZKF7fgv
```

This next line [`cat`][cat]s out the password for the next level, the `bandit22` user password. It redirects the standard output to that `/tmp/t7O6lds9S0RqQh9aMcz6ShpAoZKF7fgv` file. So that means... that that `/tmp/t7O6lds9S0RqQh9aMcz6ShpAoZKF7fgv` file contains the password for the next level, right? _And it is world-readable?_
...
__The password for bandit22 is `Yk7owGAcWjwMVRwrTesJEwB7WVOiILLI`.__
