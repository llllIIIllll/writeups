...
_John Hammond_ | _Saturday, January 9th, 2016_ 
...
We've still got the `data.txt` file to work with just like we did in the [last level](../level7/). 
...
It looks just like a bunch of random keys on every line. We've got to find _our_ key!

This time around though, we need the line of text that occurs just once. Well, since [`grep`][grep] is more of a "search", it doesn't entirely help us here... what else can we use?

If you actually look through the page in the listing of "commands you may need to solve this level" (hopefully you've been looking through all of these from the get-go), you may see one interesting that sounds like it come be something to help us. "[`uniq`][uniq]!"

Check out the [`man`][man] page for [`uniq`][uniq] and do some reading to try and piece together what to do in this situation on your own.

```
man uniq
```

So as you have hopefully read, [`uniq`][uniq] will omit repeated lines. Well that sounds awesome, it's exactly what we need, right? Let's try it out.

Let's do some [redirection].

```
cat data.txt | uniq
```

Uh.... that did literally nothing for us. What the heck?

Check out the [`man`][man] page one more time. 

Notice in the description, however, it is only filters out _adjacent_ matching lines. So we've got to sort all the data and get all the repeated lines together. That should be an easy fix, right? You should have seen "[`sort`][sort]" as another possible command to help with this challenge. Check out that [`man`][man] page and try it out.

```
cat data.txt | sort
```

Oooh! That looks much better, right? Now we can chain our [pipe]s together and run [`uniq`][uniq] on that output.

```
cat data.txt | sort | uniq
```

... It looks like that put us back where we started. Let's review the [`man`][man] of [`uniq`][uniq] again.

I would hope at this point you have read the line: __"With no options, matching lines are merged to the first occurrence."__

That is _not_ the functionality we want. But it looks like there are some [arguments] that can help us... find it? `-u`, or `--unique` should _only_ print unique lines.
...
Hopefully this is a lesson you should __always__ read the [`man`][man] pages really thoroughly.

__The password for bandit9 is `UsvVyFSfZZWbi6wgC7dAFyFuR6jQQUhR`.__
