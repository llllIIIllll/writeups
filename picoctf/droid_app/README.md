...
This challenge gives you a program for the [Android][Android] [operating system] in the form of a [`.apk`][.apk] file. This is a package of code for [Android] devices, written in [Java]. Typically, we can get the source code of some [Android] software by using a [decompiler] on the [`.apk`][.apk] file. I recommend you do this for every [CTF] challenge you encounter that asks you to work with [Android] stuff.

There are a ton of resources online to try and [decompile Java code], and there are naturally a [lot](http://www.decompileandroid.com/) [that](http://www.javadecompilers.com/apk) try and decompile [Android] software. I've had the most success so far with a tool called [`jadx`][jadx], a [command-line] tool to produce [Java] source code from [Android]  [`.apk`][.apk] files.

So download the [`.apk`][.apk] file, and run [`jadx`][jadx] on it. It should create a new directory with the same name as the [`.apk`][.apk] file and there you can poke through all the source code. 

```
$ jadx ToasterBot.apk
```

Move into the new `ToasterBot` directory, and see what we have to work with. I see a `picoapp453` folder, so the good stuff must be in there. Let's roll through to where source code will be, in `picoapp453/picoctf/com/picoapp/` (just follow through all the directories).

Not wasting any time, check out the `ToasterActivity.java` file. That looks like it has some interesting stuff... and that `mystery` string variable looks to contain our flag!

![Solution](solution.png)

__Submit: `what_does_the_logcat_say`__
