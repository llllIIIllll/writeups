...
Hmm, a [`.img`][.img] file! With these, your knee-jerk reaction should be to throw some tools at it. Download the file and lets start poking at it.

The first thing I thought to try was [`binwalk`][binwalk] to try and extract stuff out of the file system. Let's do that...
...
Let's try another tool commonly used with [disk image] files, [`foremost`][foremost].

![Solution](solution.png)

Looks like we got all the [JPG] files out! I run [`eog`][eog] just to display the images.
...
__Submit: `i_can_has_cheezburger`__
