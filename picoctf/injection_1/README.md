...
Admittedly there may be a much more "academic" way to solve this challenge, but I just threw the [URL] at [`sqlmap`][sqlmap] and it was able to find a [time-based vuilnerability][time-based]. It took a little bit, but I was able to find the admin credentials. After logging in with those, we got the flag.

Note that the database name and the table name can be discovered by simply looking at [the given source code](http://web2014.picoctf.com/injection1/login.phps).

```
$ sqlmap --form -u "http://web2014.picoctf.com/injection1/" -D sql1 -T users -C username,password --dump
```

![Solution](solution.png)

__Submit: `flag_vFtTcLf7w2st5FM74b`__
