...
Supposedly this program will do whatever we want it to. Uhh, okay, this should be easy then, right? We're given [source code](execute.c), so let's take a look.
...
Reading through it, it looks like the program first calls this `be_nice_to_people` function, which elevates our privileges. This must be so we can read the flag on their server. 

Then the program reads our input, and throws a [function pointer] at it to treat it like executable code, and runs it.

Well, jeez, looks like the program really does do whatever we want. Let's start to poke at it.

Connect to the server with [`ssh`][ssh] and `cd` into the correct directory: the challenge prompt tells us it should be in `/home/execute`.

Let's invoke the program and try to execute some code.
...
Oh wait. The program expects [machine code]. It's a [compiled][compiler] program obviously written in [C], a [compiled language]. It's not like a [scripting language] where we can just execute statements of code like that.

So.... we've got to throw [shellcode] at it!

We can find a ton of [shellcode] over on [shell-storm.org], in the [shellcode database]. Find under the '[Intel 86]' section of the [Linux] something that will execute [`/bin/sh`][/bin/sh] for us and get us a [shell]. We could also generate some on our own with [`gdb-peda`][gdb-peda], like so...
...
iEther way we get the [shellcode], now we've got our exploit that will give us a [shell]. Let's try it: 
...
Uh, what the heck? Another [segfault]? Well kmachine codeeep in mind we haven't really given it the [machine code]... all we did was give it a string with some of the escaped values. So, we'll need them to be interpretered literally; we can do this like we always do: with [Python].

```
$ python -c "print '\x31\xc0\x50\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\x31\xc9\x89\xca\x6a\x0b\x58\xcd\x80'" | ./execute
$
```

Hmmm. It looks like we're being thrown right out of the program. It looked like our [shell] executed though. Remember how we can remedy this? We can trap our [standard input] with [`cat`][cat].

```
$ ( python -c "print '\x31\xc0\x50\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\x31\xc9\x89\xca\x6a\x0b\x58\xcd\x80'" ; cat ) | ./execute
```

No output; but we can type! Run commands! `ls`, and snag that flag!

![Solution](solution.png)

__Submit: `shellcode_is_kinda_cool`__
