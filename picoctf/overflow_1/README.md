...
You can follow the links to access a [shell] from within your browser and interact with the program there; it even gives you a nice little breakdown of how to solve the problem and a visual display of how your input is affecting the program.

So yes, this is a very simple [buffer overflow] challenge; but we don't need to enter any [shellcode], the program looks like it will give us [shell] all on its own if we can just overwrite the value. That should be pretty easy; in the [source code](overflow1.c) we can see the size of the buffer and the [hex] value it is looking for.

Normally, we're overflowing a return address, so we have to account for the [stack frame] pointer in the calculation of offset ... but we don't even have to do that, since we're just overflowing the next local variable in [the stack]. ( The `secret` variable, as you can see from the if statement.)

According to the [source code](overflow1.c), we just to cover up `16` bytes and then throw in our `0xc0deface` [hex] value. Easy!

To take care of the [little-endian]ness, I just threw that [hex] value at [Python]'s `pack` function within the [`struct`][struct] module.

``` python
>>> import struct
>>> struct.pack( "<I", 0xc0deface )
'\xce\xfa\xde\xc0'
```

Now that we have that address, all we need to do is pass in some garbage to fill up the buffer. We can finish up this exploit by calling the program like so:

```
$ ./overflow1 `python -c "print 'a'*16 + '\xce\xfa\xde\xc0'"`
```

We then get our [shell], and we can `ls` and `cat flag.txt`!

__Submit: `ooh_so_critical`__
