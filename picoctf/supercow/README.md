...
Change into the problem directory `/home/daedalus` and check out the files. You'll find there are a good amount to work with.

```
pico93033@shell:/home/daedalus$ ls
flag.txt  hint.cow  secret1.cow  secret2.cow  supercow  supercow.c
```

I see the program, some source code, a few files and even a flag! 

So the `supercow` program is what we have to work with. Let's try and run it. 

```
pico93033@shell:/home/daedalus$ ./supercow 
Usage: super_cow [cow_file]
```

Looks like we need to give it a 'cow_file'. Like `secret1.cow` or `secret2.cow`?

```
pico93033@shell:/home/daedalus$ ./supercow secret1.cow 
 ____________
< cow_text_1 >
 ------------
        \   ^__^
...
Ah, yes, [`cowsay`][cowsay]. Can we just use it to get the flag?

```
pico93033@shell:/home/daedalus$ ./supercow flag.txt 
Filename must end in .cow
```

Mmm, guess not. File extension must be `.cow`. Can we rename it?

```
pico93033@shell:/home/daedalus$ mv flag.txt flag.cow
mv: cannot move ‘flag.txt’ to ‘flag.cow’: Permission denied
``` 

Oh, no, not that either. We don't have any permissions. We could have figured all of this out by looking at the source code, but I had more fun just messing around.

Well, we need this file with a `.cow` extension, but we can't rename it or anything... so, we could create a [symbolic link]?

Let's put one in a directory where we actually have write permission. __And don't forget! Whenever you are creating a [symbolic link], _always_ use [absolute path]s!__

![Screenshot](screenshot.png)

__Submit: `cows_drive_mooooving_vans`__
