...
This challenge does require a bit of reading; but because of that, you learn a lot, so it's really a good thing.

When you `cd` into `/levels` and fire up `level02`, it tells you simply that `source code is available in level02.c`. Oh, awesome. When you [`ls`][ls], you'll see there is a good amount of source code, some available for each level as appropriate. That is a good thing to keep in mind.
...
Mmm, so the jist of it, in the `main` function... if we supply it two [arguments], it will set up a handler for a [SIGFPE][SIGFPE] [signal] and then divide the two [arguments] supplied. _If_ this [SIGFPE][SIGFPE] [signal] is caught, we win and get our [shell].

Alright, so what is this [SIGFPE][SIGFPE] [signal] thing?

If you have not already done so, I recommend you read through `man signal`. If you're antsy and just want the answer, feel free to search for (with a `/` forward slash) `SIGFPE`. 

The important note that you should hopefully stumble upon is this: __(Also dividing the most negative integer by -1 may generate SIGFPE.)__

Mmm, sounds like that is exactly what we need! Now what is the most negative integer? Never forget that [Google] is [your best friend.](http://stackoverflow.com/questions/11243014/why-the-absolute-value-of-the-max-negative-integer-2147483648-is-still-2147483)

The number is `-2147483648`. Let's go ahead and divide that `-1`... _and win!_
...
__The password for level3 is `OlhCmdZKbuzqngfz`.__
