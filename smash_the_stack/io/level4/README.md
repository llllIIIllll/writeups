...
_John Hammond_ | _Tuesday, January 24th, 2016_ 
...
Let's `cd` right into `/levels` and fire up `level04`. I was immediately greeted with
...
Hmm! We're currently logged in as user `level4`, remember, not `level5`... but all of these programs work as a [`seteuid`][seteuid] binary, so within the program we have the rights of the next level.

But all it did was print out our username. Is that all it does? As a knee-jerk reaction I threw the things at [`ltrace`][ltrace], and, well, yup, it looks like that is all it does.

```
$ ltrace ./level04
__libc_start_main(0x804849c, 1, 0xbffffd94, 0x8048510, 0x8048500 <unfinished ...>
popen("whoami", "r")                                             = 0x0804a008
fgets("level4\n", 1024, 0x804a008)                               = 0xbffff8dc
printf("Welcome %s", "level4\n"Welcome level4
)                                 = 15
+++ exited (status 0) +++
```

Since we are within [`ltrace`][ltrace] there, we don't have our [`seteuid`][seteuid] in effect so we don't have the rights of `level5` and don't have all the functionality of the program; you can see we get that `level4` as our username.

We can see the [source code](level4.c) for this challenge, and it really just confirms everything we have already learned.
...
Let's think about this code. That [C] function is just running [`popen`][popen], a function that can be essentially read as "process open"... it just opens a process. All it is doing in this case is running the `whoami` command. 

The machine would find the `whoami` command within the `$PATH` [environment variable] and then just run it. How can we exploit that?

Well, what is to stop us from tampering with the `$PATH` environment variable? We could _trick_ the program into instead of running the _actual_ `whoami` command, and instead run our own, _custom-made_ rendition of `whoami`... just an executable that appears before the original in the `$PATH` [environment variable] and displays our flag.

Let's hop over to a [temporary folder] and then start to write our own version of the `whoami` command. Initially, I just made this a simple [`bash`][bash] script.
...
Now let's add this [temporary directory] to our `$PATH` [environment variable] so it will call this instead of the actual `whoami` command. Obviously, use the name of whatever directory you created; not mine. :) 

```
$ export PATH=/tmp/johns:$PATH
```

Remember the the delimeter in the `$PATH` [environment variable] is a `:` colon, and I put my [temporary directory] in the front before the original `$PATH` (I keep it so all the other commands still work, obviously) so the `whoami` executable that we wrote will be found in the `$PATH` first.

We can run the script, but we obviously don't have read [permission] on the password file as the `level4` user, so we'll just get an error. See?
...
Anyway, let's try it with the actual `level04` program. 
...
Uh, wut? __Why didn't it work?__

So I did a little bit of research on this and apparently you are not able to inherit the [`seteuid`][seteuid] permissions when calling a _script_ through a program. As a security measure (we can't complain about that too much, that is really a good thing), [Linux] won't trust it if it has a [shebang line].

Whatever, that's not really much of a problem... we can just [compile] an executable for it to run, rather than using a script.

Since all we needed to do was run a command, I got lazy and just modified the code for the original level. ;)
...
int main() {
        char username[1024];
        // All we really needed to do was change the command to run
        FILE* f = popen("cat /home/level5/.pass","r");
        fgets(username, sizeof(username), f);
        // I just stripped out that "Welcome" prefix, is all
        printf("%s", username);
...
Now that it is a full executable, we can run it and get the error message that we do not have the [permissions] to read the file; so it works as it intended, now we just have to run it through the `level04` binary so we get our [`seteuid`][seteuid] powers.
...
Good stuff. So, this level was a good learning experience; let the moral of the story be __that you cannot run a script with the same [`seteuid`][seteuid] permissions _through_ a [`seteuid`][seteuid] binary; you must run an executable to get the permissions!__

__Note that the password may have changed since this writeup; [Smash the Stack] does rotate their passwords.__

__The password for level5 is `DNLM3Vu0mZfX0pDd`.__
